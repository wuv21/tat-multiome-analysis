plotBrowserTrack2 <- function (ArchRProj = NULL, region = NULL, groupBy = "Clusters", 
  useGroups = NULL, sampleLabels = "Sample", plotSummary = c("bulkTrack", 
    "featureTrack", "loopTrack", "geneTrack"), sizes = c(10, 
      1.5, 3, 4), features = getPeakSet(ArchRProj), loops = getCoAccessibility(ArchRProj), 
  geneSymbol = NULL, useMatrix = NULL, log2Norm = TRUE, upstream = 50000, 
  downstream = 50000, tileSize = 250, maxCells = 500, minCells = 25, 
  normMethod = "ReadsInTSS", highlight = NULL, highlightFill = "firebrick3", 
  ...
  title = "", verbose = TRUE, logFile = createLogFile("plotBrowserTrack")) 
{
  ...
  .validInput(input = useGroups, name = "useGroups", valid = c("character", 
    "null"))
  .validInput(input = sampleLabels, name = "sampleLabels", 
    valid = c("character"))
  ...
  .validInput(input = features, name = "features", valid = c("granges", 
    "grangeslist", "null"))
  .validInput(input = loops, name = "loops", valid = c("granges", 
    "grangeslist", "null"))
  .validInput(input = geneSymbol, name = "geneSymbol", valid = c("character", 
    "null"))
  ...
  .validInput(input = highlight, name = "highlight", valid = c("granges", 
    "null"))
  .validInput(input = highlightFill, name = "highlightFill", 
    valid = c("character"))
  ...
  .validInput(input = facetbaseSize, name = "facetbaseSize", 
    valid = "numeric")
  ...
  .logDiffTime("Validating Region", t1 = tstart, verbose = verbose, 
    logFile = logFile)
  if (is.null(region)) {
  ...
  if (!is.null(useMatrix)) {
    featureMat <- .getMatrixValues(ArchRProj = ArchRProj, 
  ...
  ggList <- lapply(seq_along(region), function(x) {
    ...
    if ("bulktrack" %in% tolower(plotSummary)) {
      ...
      plotList$bulktrack <- .bulkTracks(ArchRProj = ArchRProj, 
        ...
        sampleLabels = sampleLabels, threads = threads, 
        maxCells = maxCells, minCells = minCells, pal = pal, 
        ...
        highlight = highlight, highlightFill = highlightFill, 
        logFile = logFile) + theme(plot.margin = unit(c(0.35, 
          0.75, 0.35, 0.75), "cm"))
    }
    if ("sctrack" %in% tolower(plotSummary)) {
      .logDiffTime(sprintf("Adding SC Tracks (%s of %s)", 
        x, length(region)), t1 = tstart, verbose = verbose, 
        logFile = logFile)
      plotList$sctrack <- .scTracks(ArchRProj = ArchRProj, 
        region = region[x], tileSize = tileSize, groupBy = groupBy, 
        sampleLabels = sampleLabels, threads = threads, 
        minCells = 5, maxCells = scCellsMax, pal = pal, 
        baseSize = baseSize, borderWidth = borderWidth, 
    ...
    if ("featuretrack" %in% tolower(plotSummary)) {
      if (!is.null(features)) {
        ...
        plotList$featuretrack <- .featureTracks(features = features, 
          ...
          hideX = TRUE, title = "Peaks", highlight = highlight, 
          highlightFill = highlightFill, logFile = logFile) + 
    ...
    if ("genetrack" %in% tolower(plotSummary)) {
      ...
      plotList$genetrack <- .geneTracks(geneAnnotation = geneAnnotation, 
        region = region[x], facetbaseSize = facetbaseSize, 
        title = "Genes", highlight = highlight, highlightFill = highlightFill, 
        logFile = logFile) + theme(plot.margin = unit(c(0.1, 
          0.75, 0.1, 0.75), "cm"))
    }
    ...
    if (!is.null(useMatrix)) {
      ...
      tryCatch({
        ...
        .logError(e, fn = "ggAlignPlots", info = "", 
          errorList = NULL, logFile = logFile)
      })
    }
  })
